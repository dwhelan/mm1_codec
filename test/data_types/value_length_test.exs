defmodule MMS.ValueLengthTest do
  ...
  describe "decode/1" do
    ...
    test "with an invalid length" do
      assert decode(<<length_quote(), 128>>) == error :value_length, <<length_quote(), 128>>, [:value_length, :uint32, :first_byte_cannot_be_128]
    end
  end
  ...
  describe "decode/2" do
    ...
    test "when insufficient bytes" do
      assert ValueLength.decode(<<1>>, &Ok.decode/1) == error :value_length, <<1>>, [:short_length, %{available_bytes: 0, short_length: 1}]
    end
  end
  ...
  describe "encode" do
    ...
    test "with an invalid integer" do
      assert encode(-1) == error :value_length, -1, [:value_length, :uint32, :out_of_range]
    end
  end
